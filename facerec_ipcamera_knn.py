...
if __name__ == "__main__":
    ...
    classifier = train("uploads", model_save_path="trained_knn_model.clf", n_neighbors=2)
    print("Training complete!")
    # process one frame in every 30 frames for speed
    process_this_frame = 29
    print('Setting cameras up...')
    # multiple cameras can be used with the format url = 'http://username:password@camera_ip:port'
    url = 'http://127.0.0.1:8080/video_stream'
            ...
            if process_this_frame % 30 == 0:
                ...
                for name, (top, right, bottom, left) in predictions:
                    if name != "unknown":
                        ...
                        utils.post_prediction(server_url, timestamp, name)
                    else:
                        print("No matching prediction")
                        server_url= 'http://127.0.0.1:8000/prediction'
                        timestamp= utils.get_now_timestamp()
                        utils.post_prediction(server_url, timestamp, "unknown")
